...
@php
    ...
    // === Mapping tampilan konsisten: Open / Progress (Verifikasi N) / Return / Sirkulir / Done ===
    $cls = 'secondary';          // warna badge
    $ico = 'bi-hourglass-split'; // ikon (bi-*)
    $lbl = 'Open';               // label utama
    $sub = null;                 // subteks (mis. "Verifikasi 1/2/3")
    ...
    if (in_array($statusRaw, ['verifikasi 1','verifikasi 2','verifikasi 3'], true)) {
        ...
        // fallback ke value asli kalau ada status lain
        $cls = 'light text-dark';
        $ico = 'bi-question-circle';
        $lbl = $review->status ?? '-';
    }

    // step terakhir (tetap)
    $lastStep = $review->steps?->sortByDesc(fn($s) => $s->tanggal ?? $s->created_at)->first();

    // giliran Manager? (logika asli dipertahankan)
    $lastTahap  = strtolower($lastStep->tahapan ?? '');
    $lastStatus = strtolower($lastStep->status ?? '');
    $stage      = strtolower($review->current_stage ?? '');
    $isManagerTurn = ($stage === 'manager')
        || in_array($statusRaw, ['verifikasi 1','verifikasi1','verifikasi-1'])
        || ($lastTahap === 'manager' && in_array($lastStatus, ['pending','menunggu']));
    ...
    // Fix: HINDARI skip()/OFFSET. Ambil semua lalu take/slice di koleksi. ===
...
<div class="container mt-5">
  <h4 class="fw-bold text-center mb-4">Detail Dokumen - Manager</h4>
    ...
    <div class="card-body">
      <div class="row">
        <div class="col-md-6">
          ...
          <div class="mb-2"><strong>Tanggal Masuk:</strong> <span class="text-primary">{{ optional($review->tanggal_masuk ?? $review->created_at)->format('Y-m-d H:i:s') ?? '-' }}</span></div>
        </div>
        <div class="col-md-6">
    ...
    <div class="card mb-4">
      ...
      <div class="card-body">
        ...
        @if ($statusRaw === 'sirkulir')
          <form action="{{ route('approval.manager.sirkulir.upload', $review->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
  ...
  <div class="card mb-4">
    <div class="card-header bg-light d-flex justify-content-between align-items-center">
      ...
      @if($bpoCount > 2)
        <button class="btn btn-sm btn-outline-secondary" type="button"
                data-bs-toggle="collapse" data-bs-target="#bpoAllManager{{ $review->id }}">
    ...
    <div class="card-body">
      @if($bpoTwo->count())
        ...
        @if($bpoCount > 2)
          <div class="collapse mt-3" id="bpoAllManager{{ $review->id }}">
            <div class="table-responsive">
              <table class="table table-sm table-bordered align-middle">
                <thead class="table-light">
                  <tr>
  ...
  <div class="card mb-4">
    <div class="card-header bg-light d-flex justify-content-between align-items-center">
      <h6 class="mb-0"><i class="fas fa-file-pdf me-2"></i>Form Hasil Review & Executive Summary</h6>
      @if($hasPdf)
    ...
    <div class="card-body">
      @if ($hasFR || $hasES)
        <div class="mb-3">
          @if ($hasFR)
            <a href="{{ Storage::url($formReviewLatest->file_path) }}" target="_blank" class="btn btn-sm btn-outline-primary me-2">
              <i class="bi bi-eye me-1"></i> Lihat Form Review (Manager)
            </a>
          @endif
          @if ($hasES)
            <a href="{{ Storage::url($executiveSummaryLatest->file_path) }}" target="_blank" class="btn btn-sm btn-outline-primary">
  ...
  @if($isManagerTurn)
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form action="{{ route('approval.manager.setujui', $review->id) }}" method="POST">
          @csrf
          <div class="mb-3" style="max-width: 420px;">
            <label class="form-label fw-bold">Status Persetujuan</label>
            <select name="status" class="form-select" required>
              <option value="Setuju">✅ Setuju (Kirim ke AVP)</option>
          ...
          <div class="d-flex gap-2">
            ...
            <a href="{{ route('approval.manager.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left me-2"></i>Kembali</a>
          </div>
        </form>
      </div>
    </div>
